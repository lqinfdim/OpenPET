...
import os
from openpet.pet_configs import BasePETConfig
from openpet.utils.inspect import inspect_module_statistics
from openpet.utils.model_md5 import gen_model_hash
from openpet.utils.signature import get_arg_names, signature
from typing import Optional, Union
from openpet.utils.cuda import get_device
from openpet.utils.name_based_addressing import *
import torch.nn as nn
import torch
from functools import wraps
# from decorator import decorate
from openpet.utils.decorate import decorate
from openpet.utils.structure_mapping import transform
# from transformers.file_utils import PushToHubMixin
# from transformers.deepspeed import deepspeed_config, is_deepspeed_zero3_enabled
from openpet import SaveLoadMixin
from openpet import logging
from openpet.utils.structure_mapping import CommonStructureMap
# from openpet.utils.interactive.web import interactive
from openpet.utils.data_parallel import new_replicate_for_data_parallel
from openpet.utils.cuda import move_dict_to_cuda
import sys

from openpet.utils.data_parallel import caller_map
from openpet.utils.backend import BackendMapping
...
class PETBase(nn.Module, SaveLoadMixin):
    r"""This is the base class for all PET models. It provides four simple but effective functionalities
    for building the PET model:
    ...
    Class attributes (overridden by derived classes):
        ...
        - config_class (:class:`BasePETConfig`): The corresponding config model
    ...
    def forward(self, *args, **kwargs) -> RuntimeError:
        ...
        """
        raise RuntimeError("This is a PET model, which should be attached to a backbone model \
            and can't forward any data by itself. Please using the backbone model's forward function \
            after attach the delta model to the backbone. ")

    @classmethod
    def from_config(cls, config: Union[BasePETConfig, dict], backbone_model: nn.Module, check_hash=True, **kwargs):
        ...
        """
        supported_keys = get_arg_names(cls.__init__) + get_arg_names(PETBase.__init__)
    ...
    def add_all_pet_to_backbone(self,
                 backbone: nn.Module,
    ...
    def mark_as_pet(self, module: nn.Module=None,):
    ...
    def _register_pet_infos(self, parent_module, _delta_info):
    ...
    def replace_module(self,
        ...
        _pet_info = {"method": "replace",
                      ...
                      "delta_belong": self,
                      "state": "on"}
        self._register_pet_infos(parent_module=parent_module,
                                   _delta_info = _pet_info,
                                  )
    ...
    def insert_module(self, module, method='sequential', delta_module=None, delta_name='delta', strict=False, _delta_info=None):
        ...
        if _delta_info is None:
            ...
            _delta_info = {"method": method,
                        ...
                        "delta_belong": self,
                        "state": "on"}
            self._register_pet_infos(parent_module=module,
    ...
    def create_config_from_model(self, ):
        ...
        config.delta_type = self.pet_type
        self.config = config
